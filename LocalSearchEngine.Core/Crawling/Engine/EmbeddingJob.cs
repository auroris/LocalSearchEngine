...
{
    /// <summary>Leave this URL's chunks as they are (an unchanged page); only a redirect source, if any, is cleared.</summary>
    None,
    /// <summary>Remove this URL's chunks (the page is no longer indexed: noindex, gone, or a canonical alias).</summary>
    Delete,
...
/// <c>text_chunks</c> into line. <see cref="Action"/> covers the URL's own chunks, while
/// <see cref="RedirectSourceUrl"/>, when set, names a dequeued URL whose chunks must also be cleared
/// because the fetch redirected to <see cref="Url"/>. Mirrors exactly the chunk writes the old single
/// consumer did per job type.
/// </summary>
/// <param name="Url">The (final) page URL whose chunks the action applies to.</param>
/// <param name="RedirectSourceUrl">A dequeued URL whose chunks to delete when a redirect was followed; otherwise <c>null</c>.</param>
...
internal sealed record EmbeddingJob(string Url, string? RedirectSourceUrl, ChunkAction Action, string Text, string Headings)
{
    /// <summary>
    /// Builds the chunk work implied by a finished <see cref="CrawlJob"/>, or <c>null</c> when the job
    /// touches no chunks at all (a plain unchanged page with no redirect) and need not be queued.
    /// </summary>
    ...
    public static EmbeddingJob? From(CrawlJob job)
    {
        ...
        // An unchanged page that did not redirect leaves text_chunks untouched — don't bother the queue.
        if (action == ChunkAction.None && job.RedirectSourceUrl is null)
        {
            return null;
        }

        var (text, headings) = job is IndexJob index ? (index.Text, index.Headings) : (string.Empty, string.Empty);
        return new EmbeddingJob(job.Url, job.RedirectSourceUrl, action, text, headings);
    }
}
